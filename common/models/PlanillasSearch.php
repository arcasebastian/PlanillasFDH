<?php

namespace common\models;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use common\models\Planillas;

/**
 * PlanillasSearch represents the model behind the search form of `common\models\Planillas`.
 */
class PlanillasSearch extends Planillas
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['id', 'activo'], 'integer'],
            [['descripcion', 'linea', 'rubro', 'marca'], 'safe'],
            [['costo_sin_impuesto', 'costo_con_impuesto', 'precio_mayorista', 'efectivo', 'porcentaje_iva'], 'number'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = Planillas::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'id' => $this->id,
            'costo_sin_impuesto' => $this->costo_sin_impuesto,
            'costo_con_impuesto' => $this->costo_con_impuesto,
            'precio_mayorista' => $this->precio_mayorista,
            'efectivo' => $this->efectivo,
            'porcentaje_iva' => $this->porcentaje_iva,
            'activo' => $this->activo,
        ]);

        $query->andFilterWhere(['like', 'descripcion', $this->descripcion])
            ->andFilterWhere(['like', 'linea', $this->linea])
            ->andFilterWhere(['like', 'rubro', $this->rubro])
            ->andFilterWhere(['like', 'marca', $this->marca]);

        return $dataProvider;
    }
}
